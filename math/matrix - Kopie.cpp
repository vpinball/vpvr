...
vec4 vec4::normal(vec4 &input) {
   ...
   len = 1.0f / sqrt(len);
   retrun vec4(input.x*len, input.y*len, input.z*len, input.w*len);
}

vec4 vec4::dot(vec4 &a, vec4 &b) {
...
vec3 vec3::normal(vec3 &input) {
   ...
   len = 1.0f / sqrt(len);
   retrun vec3(input.x*len, input.y*len, input.z*len);
}
