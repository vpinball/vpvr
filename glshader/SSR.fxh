...
// uses Texture6 & Texture5,Texture5 & Texture4 & w_h_height.xy
...
float3 approx_bump_normal(float2 coords, float2 offs, float scale, float sharpness)
{
    ...
    float lpx = dot(tex2Dlod(Texture5, float4(coords.x+offs.x,coords.y, 0.,0.)).xyz, lumw);
    float lmx = dot(tex2Dlod(Texture5, float4(coords.x-offs.x,coords.y, 0.,0.)).xyz, lumw);
    float lpy = dot(tex2Dlod(Texture5, float4(coords.x,coords.y+offs.y, 0.,0.)).xyz, lumw);
    float lmy = dot(tex2Dlod(Texture5, float4(coords.x,coords.y-offs.y, 0.,0.)).xyz, lumw);

    float dpx = tex2Dlod(Texture6, float4(coords.x+offs.x,coords.y, 0.,0.)).x;
    float dmx = tex2Dlod(Texture6, float4(coords.x-offs.x,coords.y, 0.,0.)).x;
    float dpy = tex2Dlod(Texture6, float4(coords.x,coords.y+offs.y, 0.,0.)).x;
    float dmy = tex2Dlod(Texture6, float4(coords.x,coords.y-offs.y, 0.,0.)).x;
...
void main()
{
	...
	float3 color0 = tex2Dlod(Texture5, float4(u, 0.,0.)).xyz; // original pixel
	...
	for(int i=1; i</*=*/samples; i++) //!! due to jitter
	{
		...
		float3 color = tex2Dlod(Texture5, float4(offs, 0.,0.)).xyz;
