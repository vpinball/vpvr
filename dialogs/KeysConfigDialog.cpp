   ...
   if (toolTipHwnd)
   {
    ...
    for (unsigned int i = 0; i <= 34; ++i)
    {
        ...
        switch (i)
        {
            ...
            case 31:hr = LoadValue("Player"s, "JoyRoomRecenterKey"s, selected); item = IDC_JOYROOMRECENTER; break;
            case 32:hr = LoadValue("Player"s, "JoyTableRecenterKey"s, selected); item = IDC_JOYTABLERECENTER; break;
            case 33:hr = LoadValue("Player"s, "JoyTableUpKey"s, selected); item = IDC_JOYTABLEUP; break;
            case 34:hr = LoadValue("Player"s, "JoyTableDownKey"s, selected); item = IDC_JOYTABLEDOWN; break;
        }
    ...
    on = LoadValueBoolWithDefault("Player"s, "CaptureExternalDMD"s, false);
    ::SendMessage(GetDlgItem(IDC_CAP_EXTDMD).GetHwnd(), BM_SETCHECK, on ? BST_CHECKED : BST_UNCHECKED, 0);

    on = LoadValueBoolWithDefault("Player"s, "CapturePUP"s, false);
...
void KeysConfigDialog::OnOK()
{
    ...
    SaveValueBool("Player"s, "CaptureExternalDMD"s, selected != 0);

    selected = ::SendMessage(GetDlgItem(IDC_CAP_PUP).GetHwnd(), BM_GETCHECK, 0, 0);
    SaveValueBool("Player"s, "CapturePUP"s, selected != 0);
