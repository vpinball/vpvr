...
void Bumper::RenderBase(const Material * const baseMaterial)
{
   ...
   pd3dDevice->basicShader->SetTexture(SHADER_Texture0, &m_baseTexture, false, false);
...
void Bumper::RenderSocket(const Material * const socketMaterial)
{
   ...
   pd3dDevice->basicShader->SetTexture(SHADER_Texture0, &m_skirtTexture, false, false);
   g_pplayer->m_pin3d.EnableAlphaBlend(false);
   pd3dDevice->basicShader->SetAlphaTestValue((float)(1.0 / 255.0));
...
void Bumper::RenderCap(const Material * const capMaterial)
{
   ...
   pd3dDevice->basicShader->SetTexture(SHADER_Texture0, &m_capTexture, false, false);
   g_pplayer->m_pin3d.EnableAlphaBlend(false);
   pd3dDevice->basicShader->SetAlphaTestValue((float)(1.0 / 255.0));
...
void Bumper::RenderDynamic()
{
   ...
   if (m_d.m_ringVisible)
   {
      ...
      pd3dDevice->basicShader->SetTexture(SHADER_Texture0, &m_ringTexture, false, false);
